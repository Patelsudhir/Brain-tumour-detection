...
if DATA_SIZE == 'all':
    ...
    HGG_path_list = tl.files.load_folder_list(path=HGG_data_path)[0:0] # DEBUG WITH SMALL DATA
    LGG_path_list = tl.files.load_folder_list(path=LGG_data_path)[0:40] # DEBUG WITH SMALL DATA
else:
...
for i in LGG_name_list:   #changes are made here for only LGG
        ...
        for i in range(batch_size):
            if np.max(b_images[i]) > 0:
                ...
                file_metadata = {
                  ...
                  'mimeType': 'image/png',
                  'parents': ['1RUMmetEgsF77Hla-5GtFg_bj5OV1WAys']
                }
